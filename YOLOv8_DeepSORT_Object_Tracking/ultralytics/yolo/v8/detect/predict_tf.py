...
def draw_boxes(img, bbox, names,object_id, identities=None, offset=(0, 0)):
    ...
    for i, box in enumerate(bbox):
        ...
        if obj_name == "zebra":
            y_max = y2 if y2 > y_max else y_max     #top=>0 min=>max 取畫面最下方的斑馬線
            line[0]=(int(width*0.2),y_max)
            line[1]=(int(width*0.8),y_max)
        if obj_name == "traffic light":
            stop_flag = read_traffic_lights_object(img,x1, y1, x2, y2)
    for i, box in enumerate(bbox):
        x1, y1, x2, y2 = [int(i) for i in box]
        x1 += offset[0]
        x2 += offset[0]
        ...
        if len(data_deque[id]) >= 2:
          ...
          if intersect(data_deque[id][0], data_deque[id][1], line[0], line[1]):
              if ("South" in direction) & (obj_name!="zebra")&(stop_flag==False):
                log_flag = True
                if (obj_name not in object_counter) :
                    object_counter[obj_name] = 1
                else:
                    object_counter[obj_name] += 1
              if ("North" in direction) & (obj_name!="zebra")&(stop_flag==False):
                log_flag = True
        ...
        if stop_flag==False: #紅燈
            cv2.putText(img, 'Stop', (15, 25), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 1)
        else:   #stop_flag==True
            cv2.putText(img, 'Go', (15, 25), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 1)
